...
@st.cache_resource
def load_model():
    # Using Padim with ResNet18 backbone (lightweight)
    model = get_model(model_name="padim", backbone="resnet18", pretrained=True)
    model.eval()
    return model
...
def detect_defects_anomalib(image):
    ...
    # Normalize and resize anomaly map
    ...
    for cnt in contours:
        ...
        if area > 100:
            ...
            # Draw white rectangle around defect
...
if pasted_image is not None:
    image = cv2.cvtColor(np.array(pasted_image), cv2.COLOR_RGB2BGR)
